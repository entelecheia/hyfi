    ...
    * ``hf`` for objects or repositories on `HuggingFace Hub`_.
    ...
    if parsed.scheme in get_supported_schemes():
        ...
        if url_or_filename.exists():
            ...
            # Something unknown
            raise ValueError(
                f"unable to parse {orig_url_or_filename} as a URL or as a local path"
            )
...
def get_from_cache(
    ...
    except client.recoverable_errors:
    ...
    with FileLock(_lock_file_path(cache_path), read_only_ok=True):
        if os.path.exists(cache_path):
            ...
            with CacheFile(cache_path) as cache_file:
                ...
                try:
                    display_url = (
                        url if len(url) <= 30 else f"\N{horizontal ellipsis}{url[-30:]}"
                    )
                    task_id = progress.add_task(
                        f"Downloading [cyan i]{display_url}[/]", total=size
                    )
                    writer_with_progress = BufferedWriterWithProgress(
                        cache_file, progress, task_id
                    )
